...
public class CSVImporter<T>
{
	...
	///
	public convenience init(path:String, delimiter:String = ",", lineEnding:LineEnding = .unknown, encoding:String.Encoding = .utf8, workQosClass:DispatchQoS.QoSClass = .utility, callbacksQosClass:DispatchQoS.QoSClass? = nil)
	{
	...
	///   - callbacksQosClass: The QOS class of the background queue to run the callbacks in or `nil` for the main queue. Defaults to `nil`.
	///
	public convenience init?(url:URL, delimiter:String = ",", lineEnding:LineEnding = .unknown, encoding:String.Encoding = .utf8, workQosClass:DispatchQoS.QoSClass = .utility, callbacksQosClass:DispatchQoS.QoSClass? = nil)
	{
	...
	///   - callbacksQosClass: The QOS class of the background queue to run the callbacks in or `nil` for the main queue. Defaults to `nil`.
	///
	public convenience init(contentString:String, delimiter:String = ",", lineEnding:LineEnding = .unknown, workQosClass:DispatchQoS.QoSClass = .utility, callbacksQosClass:DispatchQoS.QoSClass? = nil)
	{
	...
	public func startImportingRecords(mapper closure:@escaping (_ recordValues:[String]) -> T) -> Self
	{
		workDispatchQueue.async
		{
			...
			let importedLinesWithSuccess = self.importLines
			{ valuesInLine in
	...
	/// - Returns: The imported records array.
	///
	public func importRecords(structure structureClosure:@escaping (_ headerValues:[String]) -> Void, recordMapper closure:@escaping (_ recordValues:[String:String]) -> T) -> [T]
	{
	...
	func readValuesInLine(_ line:String) -> [String]
	{
		...
		components = components.map
		{
			$0.replacingOccurrences(of: substitute, with: "\"")
		}
...
class FileSource: Source
{
	...
	func forEach(_ closure:(String) -> Void)
	{
		...
		guard let csvStreamReader = textFile.streamReader(lineEnding: lineEnding, chunkSize: chunkSize) else
		{
			return
		}
