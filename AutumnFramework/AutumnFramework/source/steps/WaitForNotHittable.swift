...
/**
 * A test step that waits for the specified element to not be hittable within a specified timeout.
 */
public class WaitForNotHittable : AutumnUITestStep
{
	// ----------------------------------------------------------------------------------------------------
	// MARK: - Properties
	// ----------------------------------------------------------------------------------------------------
	...
	{
		if name.isEmpty { name = "\(elementName) becomes unhittable within \(_timeout) seconds" }
	}
	
	
	public override func execute() -> AutumnTestStepResult
	{
		result.add("Wait for [\(id)] to not be hittable within \(_timeout) seconds", AutumnUI.waitForNotHittable(element, timeout: _timeout))
		return result
	}
}
