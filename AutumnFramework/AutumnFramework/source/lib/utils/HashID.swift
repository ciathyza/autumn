...
public protocol HashIDGenerator
{
	associatedtype Char
...
public typealias HashID = HashID_<UInt32>
...
open class HashID_<T>: HashIDGenerator where T : UnsignedInteger
{
	...
	public init(salt:String!, minHashLength:UInt = 0, alphabet:String? = nil)
	{
		...
		if (0 == sepsLength) || (Double(alphabetLength) / Double(sepsLength) > HashIDOptions.SEP_DIV)
		{
			...
			var newSepsLength = Int(ceil(Double(alphabetLength) / HashIDOptions.SEP_DIV))
